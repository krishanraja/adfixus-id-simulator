...
Index.tsx → GuidedFlow ──owns──► useIdSimulator()  ──calls──► @/core engine
                │                       │
                │ steps 0-2             │ live results + audience-visibility story
                ▼                       ▼
   Provocation → Ask → Reveal    IdSimulator (full sim) rendered inside DepthDrawer
   (in FlowShell)                (opened from the Reveal CTA, shares the SAME instance)
```
...
- **One state instance.** `GuidedFlow` creates a single `useIdSimulator()` and
  passes it into both the guided steps and the drawer's `IdSimulator`, so the
  audience size chosen in the flow carries into the full picture and back.
...
|---------------------|------|
| The guided-flow copy / steps | `src/components/flow/GuidedFlow.tsx` (+ `Provocation`, `AskStep`, `Reveal`) |
