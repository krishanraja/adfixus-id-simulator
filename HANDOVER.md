...
Index.tsx → GuidedFlow ──owns──► useIdSimulator()  ──calls──► @/core engine
                ...
                ▼                       ▼
Provocation → Domain → Ask → Reveal   IdSimulator (full sim) inside DepthDrawer
...
the profile into `TailoredBriefing` (shown compact on the Reveal, full atop the
drawer). See core spec §6.
